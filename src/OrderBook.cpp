...
bool OrderBook::addOrder(const Order& order)
{
    ...
    if (order.type == Type::New)
    {
        ...
        if (working.quantity == 0)
            return true; // fully filled, nothing to rest

        // Insert remaining as resting order
        if (working.side == Side::Buy)
        {
            auto& level = _bids[working.price];
            level.push_back(working);
            auto it = std::prev(level.end());
            _lookup[working.orderId] = it;
        }
        else
        {
